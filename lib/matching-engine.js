...
// ── Safe tags parser (handles both array and string-encoded JSON) ─────────
...
function extractImplicitSkills(job) {
  const text = [job.description || '', job.requirements || '', safeTags(job.tags).join(' ')].join(' ');
...
function scoreCategory(candidate, job, jobCriteria, reasons, gaps, insights) {
  ...
  const jobTags = safeTags(job.tags).map(normalizeText);
...
function calculateCandidateJobMatch(candidate, job, jobCriteria, customWeights) {
  ...
  const hasJobText = !!(job.description || job.requirements || (safeTags(job.tags).length > 0));
