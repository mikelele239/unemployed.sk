...
/**
 * Fetch all conversations for the current employer/recruiter.
 */
...
/**
 * Create or get conversation for a candidate invitation.
 */
export const getOrCreateCandidateInviteConversation = async (candidateId, jobId) => {
  const { data: { session } } = await supabase.auth.getSession();
  ...
  const { data: profile } = await supabase.from('profiles').select('first_name, last_name, email').eq('user_id', candidateId).maybeSingle();
