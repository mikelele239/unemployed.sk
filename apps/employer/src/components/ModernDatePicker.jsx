...
const ModernDatePicker = ({ onSelect, onCancel }) => {
  ...
  if (sent) {
    return (
      ...
      >
        ...
        >
          {offeredDates.length} {offeredDates.length === 1 ? 'termín odoslaný' : 'termíny odoslané'}
        </motion.div>
        <motion.button
          initial={{ opacity: 0 }} animate={{ opacity: 1 }}
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: '20px' }}>
        <div style={{ fontSize: '10px', fontWeight: '800', color: '#444', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Ponúkané termíny ({offeredDates.length})</div>
      ...
      >
        {offeredDates.length === 0 ? 'Pridajte termíny' : '📨 Odoslať pozvánku'}
      </motion.button>
    </motion.div>
  );
};
