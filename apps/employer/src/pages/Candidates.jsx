...
const Candidates = () => {
  ...
  const MobileColumn = ({ col, colCandidates }) => {
            ...
            >
              <div style={{ padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                {colCandidates.length === 0 ? (
                  ...
                  colCandidates.map(c => (
                    <CandidateCard key={c.id} c={c} colColor={col.color} />
                  ))
                )}
              </div>
            </motion.div>
  ...
  return (
    <div className="kanban-fullpage" style={{ animation: 'tabSlideIn 0.3s cubic-bezier(0.4, 0, 0.15, 1)' }}>
      ...
      {/* ── Search Bar ── */}
      <div style={{
        margin: '8px 20px 16px', display: 'flex', alignItems: 'center', gap: 8,
        padding: '9px 14px', background: 'var(--bg-card)', borderRadius: 'var(--radius)', border: '1px solid var(--border)',
        flexShrink: 0
      }}>
      ...
      {!loading && (
        isMobile ? (
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 12px 24px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {columns.map(col => (
              <MobileColumn key={col.id} col={col} colCandidates={getColumnCandidates(col.id)} />
            ))}
          </div>
        ) : (
